...
**Core domain expertise.** When the Core has strong domain-specific knowledge, it resists Shell directives that conflict with that knowledge. Haiku's Core has substantial chess knowledge from pre-training data; the Shell instruction "vary openings" cannot override learned opening theory. In Trust Game, the Core has no game-theory-specific training — the cooperative prior is the primary behavioral program available, and it is easily overridden by explicit competitive directives.
